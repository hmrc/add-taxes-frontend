...
class WhichVATServicesToAddControllerSpec extends ControllerSpecBase with MockitoSugar with FeatureToggleSupport {
  ...
  def viewAsString(form: Form[_] = form, radioOptions: Seq[RadioOption] = WhichVATServicesToAdd.options, checkForVatEnrolment: Boolean = false): String =
    new whichVATServicesToAdd(formWithCSRF, mainTemplate)(frontendAppConfig, form, radioOptions, checkForVatEnrolment)(HtmlFormat.empty)(fakeRequest, messages).toString
