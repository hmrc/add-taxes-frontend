...
class WhatIsYourOrganisationControllerSpec extends ControllerSpecBase {
  ...
  val view: whatIsYourOrganisation = injector.instanceOf[whatIsYourOrganisation]

  def controller() =
    new WhatIsYourOrganisationController(
      frontendAppConfig,
      mcc,
  ...
  def viewAsString(form: Form[_] = form): String =
    new whatIsYourOrganisation(formWithCSRF, mainTemplate)(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString

  "WhatIsYourOrganisation Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", WhatIsYourOrganisation.options.head.value)).withMethod("POST")
