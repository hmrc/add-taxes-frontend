...
class DoYouWantToLeaveCISControllerSpec extends ControllerSpecBase {
  ...
  val view: doYouWantToLeaveCIS = injector.instanceOf[doYouWantToLeaveCIS]

  def controller(): DoYouWantToLeaveCISController = {
  ...
  def viewAsString(form: Form[_] = form): String =
    new doYouWantToLeaveCIS(formWithCSRF, mainTemplate)(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString

  "DoYouWantToLeaveCIS Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", DoYouWantToLeaveCIS.options.head.value)).withMethod("POST")
