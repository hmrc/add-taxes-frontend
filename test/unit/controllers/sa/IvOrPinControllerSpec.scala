...
class IvOrPinControllerSpec extends ControllerSpecBase {
  ...
  val formProvider = new IvOrPinFormProvider()
  val form: Form[IvOrPin] = formProvider()

  val view: ivOrPin = injector.instanceOf[ivOrPin]

  def controller(pinAndPostToggle: Boolean = true) = {
    new IvOrPinController(
      frontendAppConfig,
      mcc,
  ...
  def viewAsString(form: Form[_] = form): String =
    new ivOrPin(formWithCSRF, mainTemplate)(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString

  "IvOrPin Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", IvOrPin.options.head.value))
    ...
    "redirect to BTA home page when feature toggle is false" in {
