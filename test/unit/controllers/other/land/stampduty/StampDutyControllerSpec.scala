...
class StampDutyControllerSpec extends ControllerSpecBase {
  ...
  val view: stampDuty = injector.instanceOf[stampDuty]

  def controller(): StampDutyController = {
    new StampDutyController(
      frontendAppConfig,
      mcc,
  ...
  def viewAsString(form: Form[_] = form): String =
    new stampDuty(formWithCSRF, mainTemplate)(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString

  "StampDuty Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", StampDuty.options.head.value)).withMethod("POST")
