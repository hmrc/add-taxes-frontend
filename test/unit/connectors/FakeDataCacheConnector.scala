...
class FakeDataCacheConnector(val sr: SessionRepository, val cu: CascadeUpsert,  val request: ServiceInfoRequest[AnyContent])
                            (implicit override val ec: ExecutionContext) extends DataCacheConnector(sr, cu)(ec) {
  ...
  override def addToCollection[A](cacheId: String, collectionKey: String, value: A)(
    implicit fmt: Format[A]): Future[CacheMap] = Future(CacheMap(cacheId, Map()))

  override def removeFromCollection[A](cacheId: String, collectionKey: String, item: A)(
    implicit fmt: Format[A]): Future[CacheMap] = Future(CacheMap(cacheId, Map()))

  override def replaceInCollection[A](cacheId: String, collectionKey: String, index: Int, item: A)(
    implicit fmt: Format[A]): Future[CacheMap] = Future(CacheMap(cacheId, Map()))
}
