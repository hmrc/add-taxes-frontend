...
class StopCorporationTaxControllerSpec extends ControllerSpecBase {

  def onwardRoute = controllers.routes.IndexController.onPageLoad()

  val formProvider = new StopCorporationTaxFormProvider()
  ...
  def viewAsString(form: Form[_] = form) =
    stopCorporationTax(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString
  ...
  "StopCorporationTax Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", StopCorporationTax.options.head.value))
