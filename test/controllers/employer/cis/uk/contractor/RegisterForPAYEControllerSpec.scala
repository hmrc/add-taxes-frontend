...
class RegisterForPAYEControllerSpec extends ControllerSpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new RegisterForPAYEController(frontendAppConfig, messagesApi, FakeAuthAction, FakeServiceInfoAction, mockUrlHelper)

  def viewAsString() = registerForPAYE(frontendAppConfig, mockUrlHelper)(HtmlFormat.empty)(fakeRequest, messages).toString
