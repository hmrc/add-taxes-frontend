...
class DoYouWantToAddImportExportControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new DoYouWantToAddImportExportController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction,
      FakeServiceInfoAction, formProvider)

  def viewAsString(form: Form[_] = form) = doYouWantToAddImportExport(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString
