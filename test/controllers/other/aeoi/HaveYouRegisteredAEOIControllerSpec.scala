...
class HaveYouRegisteredAEOIControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new HaveYouRegisteredAEOIController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction,
      FakeServiceInfoAction, formProvider)

  def viewAsString(form: Form[_] = form) = haveYouRegisteredAEOI(frontendAppConfig, form)(HtmlFormat.empty)(fakeRequest, messages).toString
  ...
  "HaveYouRegisteredAEOI Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", HaveYouRegisteredAEOI.options.head.value))
