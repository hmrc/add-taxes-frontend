...
class HaveYouRegisteredForRebatedOilsControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[_] = form) = haveYouRegisteredForRebatedOils(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "HaveYouRegisteredForRebatedOils Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(HaveYouRegisteredForRebatedOilsId.toString -> JsString(HaveYouRegisteredForRebatedOils.values.head.toString))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad(NormalMode)(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(HaveYouRegisteredForRebatedOils.values.head))
    }
