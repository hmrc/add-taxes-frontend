...
import forms.FindingYourAccountFormProvider
import identifiers.FindingYourAccountId
import views.html.findingYourAccount
...
class FindingYourAccountController @Inject()(
                                        ...
                                        formProvider: FindingYourAccountFormProvider) extends FrontendController with I18nSupport with Enumerable.Implicits {
  ...
  def onSubmit() = (authenticate andThen serviceInfoData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        (value) =>
          Future.successful(Redirect(navigator.nextPage(FindingYourAccountId, value)))
      )
  }
}
