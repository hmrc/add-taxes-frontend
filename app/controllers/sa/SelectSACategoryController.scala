...
class SelectSACategoryController @Inject()(
                                        ...
                                        formProvider: SelectSACategoryFormProvider) extends FrontendController with I18nSupport with Enumerable.Implicits {
  ...
  def onSubmit() = (authenticate andThen serviceInfoData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        (value) =>
          Future.successful(Redirect(navigator.nextPage(SelectSACategoryId, value)))
      )
  }
}
