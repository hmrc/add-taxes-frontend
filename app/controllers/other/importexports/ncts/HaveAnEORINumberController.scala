...
class HaveAnEORINumberController @Inject()(
  ...
  formProvider: HaveAnEORINumberFormProvider
  ...
  def onPageLoad() = (authenticate andThen serviceInfoData) {
    implicit request =>
      Ok(haveAnEORINumber(appConfig, form)(request.serviceInfoContent))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(haveAnEORINumber(appConfig, formWithErrors)(request.serviceInfoContent))),
        (value) =>
          Future.successful(Redirect(navigator.nextPage(HaveAnEORINumberId, value)))
      )
  }
}