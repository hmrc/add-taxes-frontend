...
class DoYouHaveEORINumberController @Inject()(
  ...
  def onPageLoad() = (authenticate andThen serviceInfoData) {
    implicit request =>
      Ok(doYouHaveEORINumber(appConfig, form, controllers.other.importexports.ncts.routes.DoYouHaveEORINumberController.onSubmit())(request.serviceInfoContent))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(doYouHaveEORINumber(appConfig, formWithErrors, controllers.other.importexports.ncts.routes.DoYouHaveEORINumberController.onSubmit())(request.serviceInfoContent))),
        (value) =>
