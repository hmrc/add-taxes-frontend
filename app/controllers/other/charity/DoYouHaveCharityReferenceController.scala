...
class DoYouHaveCharityReferenceController @Inject()(
                                        ...
                                        formProvider: DoYouHaveCharityReferenceFormProvider) extends FrontendController with I18nSupport with Enumerable.Implicits {
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(doYouHaveCharityReference(appConfig, formWithErrors)(request.serviceInfoContent))),
        (value) =>
          Future.successful(Redirect(navigator.nextPage(DoYouHaveCharityReferenceId, value)))
      )
  }
}
